  ...
  return (
    <div className="sp-page">
      <SectionHead
        ...
        title="Boost on the map"
        sub="Put your location near the top of the StallPass map for nearby people. Pay only for the days you run."
        action={
          <Btn variant="primary" onClick={() => setShowForm(true)}>
            <IcoSpark size={14} /> New campaign
          </Btn>
        }
      />
      ...
      <div className="sp-grid-3" style={{ marginBottom: 28 }}>
        {[
          { n: '1', title: 'Pick a location', sub: 'Choose which bathroom listing to promote.' },
          { n: '2', title: 'Set your budget', sub: 'Choose how much to spend each day. Most campaigns run at $0.25-$1/day.' },
          { n: '3', title: 'Go live', sub: 'Your listing starts showing higher on the map for nearby people.' },
        ].map((step) => (
          <div
            key={step.n}
            style={{
      ...
      {campaigns.filter((c) => c.active).length === 0 ? (
        <Card style={{ marginBottom: 24 }}>
          <EmptyState
            icon={<IcoSpark size={22} />}
            title="No active campaigns"
            sub="Start a campaign to help a location stand out on the StallPass map."
            action={
              <Btn variant="primary" onClick={() => setShowForm(true)}>
                <IcoSpark size={14} /> Start a campaign
              </Btn>
            }
          />
        </Card>
  ...
  return (
    <Card style={{ opacity: faded ? 0.6 : 1 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        ...
        <Badge size="lg" tone={campaign.active ? 'success' : 'neutral'}>
      ...
      <div
        className="sp-grid-3"
        style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--surface-strong)', gap: 12 }}
      >
      ...
      {onPause && campaign.active && (
        <div style={{ marginTop: 14 }}>
          <Btn variant="secondary" size="sm" onClick={onPause}>
            Pause campaign
          </Btn>
        </div>
      )}
...
function NewCampaignModal({ onClose, onSave }) {
  ...
  const total = (parseFloat(form.budget || 0) * parseInt(form.days || '0', 10)).toFixed(2);

  function submit() {
    const today = new Date();
    const end = new Date(today);
    end.setDate(end.getDate() + parseInt(form.days || '0', 10));
    onSave({
      id: `feat-${Date.now()}`,
      ...
      <div style={{ display: 'grid', gap: 16 }}>
          ...
          <select className="sp-select" value={form.locationId} onChange={(e) => set('locationId', e.target.value)}>
            {MOCK_DATA.locations.map((location) => (
              <option key={location.id} value={location.id}>
                {location.businessName} - {location.placeName}
              </option>
            ))}
          </select>
        </div>
        ...
        >
          ...
          <div style={{ fontSize: '0.9rem', color: 'var(--ink)', lineHeight: 1.7 }}>
            <div>
              {loc?.placeName} - {form.days} day{form.days !== '1' ? 's' : ''}
            </div>
            <div>
              <strong>${form.budget}/day</strong> - <strong>${total} total</strong>
            </div>
          </div>
        </div>
      </div>
