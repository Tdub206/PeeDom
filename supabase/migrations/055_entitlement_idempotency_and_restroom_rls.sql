-- ============================================================================
-- Entitlement idempotency and restroom-intelligence write hardening.
-- ============================================================================

-- Keep the legacy live-status bridge compatible with the richer status table.
alter table public.bathroom_live_status_events
  drop constraint if exists bathroom_live_status_events_status_value_valid;

alter table public.bathroom_live_status_events
  add constraint bathroom_live_status_events_status_value_valid
  check (
    case status_type
      ...
      when 'line' then status_value in ('line', 'none', 'short', 'moderate', 'long', 'long_wait', 'line_reported')
...
create policy "bathroom_live_status_events_insert_authenticated"
  ...
  with check (
    ...
    and reported_at between now() - interval '5 minutes' and now() + interval '1 minute'
    and expires_at > reported_at
    and expires_at <= reported_at + interval '2 hours'
    and expires_at <= now() + interval '2 hours 1 minute'
    and confidence_score <= 0.8500
  );
...
begin
  ...
  where verifications.verification_token = p_reward_verification_token
