...
-- 043_server_capabilities_foundation.sql
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'business_bathroom_settings_access_instructions_len_check'
  ) then
    alter table public.business_bathroom_settings
      add constraint business_bathroom_settings_access_instructions_len_check
      check (
