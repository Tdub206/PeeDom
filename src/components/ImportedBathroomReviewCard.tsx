...
import type { ImportedLocationFreshnessStatus } from '@/types';

interface ImportedBathroomReviewCardProps {
  confirmationCount: number;
  denialCount: number;
  freshnessStatus: ImportedLocationFreshnessStatus | null | undefined;
...
function getFreshnessHeadline(status: ImportedLocationFreshnessStatus | null | undefined): string {
  switch (status) {
    case 'fresh':
...
function getFreshnessBody(status: ImportedLocationFreshnessStatus | null | undefined): string {
  switch (status) {
    case 'fresh':
    ...
    case 'unreviewed':
    default:
      return 'This listing came from OpenStreetMap. If you visited, a quick confirmation helps keep it on the map. If the restroom is gone, flag it now so stale data gets reviewed fast.';
  }
}
  ...
  return (
    <View className="mt-6 rounded-[32px] border border-brand-200 bg-brand-50 p-6">
      <View className="flex-row items-center justify-between gap-3">
        <Text className="text-sm font-semibold uppercase tracking-[1px] text-brand-700">Imported Location Review</Text>
        <BathroomOriginBadge label="OSM Import" />
      </View>
